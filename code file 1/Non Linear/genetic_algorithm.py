...
def multiple_linear_regression(X,Y):
    ...
    Y_p = np.dot(X, coeff)
    Y_mean = np.mean(Y)
    S_Mean = np.array([(i - Y_mean) ** 2 for i in Y]).sum()
    S_Predicted = np.array([(i - j) ** 2 for i, j in zip(Y, Y_p)]).sum()
    COD = (1 - (S_Predicted / S_Mean)) * 100.0 # R2 error - by john guttag
    av_error = (S_Predicted / len(Y))
...
def get_fitness(individual, inputs,outputs):
    ...
    S_Mean = np.array([(i - output_mean) ** 2 for i in outputs]).sum()
    S_Predicted = np.array([(i - j) ** 2 for i, j in zip(outputs, predicted_outputs)]).sum()
    COD = (1 - (S_Predicted / S_Mean)) * 100.0
    average_error = (S_Predicted / len(outputs))
...
# ## Testing ##########
# #####################
# data  = pd.read_csv('D.csv', header = 0) #D = pp data
...
# X = np.array(X_i)
# Y = np.array(_Y)
# a,b,c = activate(X,Y,100,10,20,0.25,0.1)
# print(c)
# #######################
# #######################
