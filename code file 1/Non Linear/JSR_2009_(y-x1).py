...
"""
time = data.iloc[:,0][1:].tolist()
_u = data.iloc[:,1][1:].tolist()
_v = data.iloc[:,2][1:].tolist()
_r = data.iloc[:,3][1:].tolist()
yaw_angle = data.iloc[:,4][1:].tolist()
_U = data.iloc[:,5][1:].tolist()
__rac_ = data.iloc[:,6][1:].tolist()
...
_rac = radian(np.array(__rac_)) # changing the values to radian mode
_rac = _rac.tolist()
...
# a,b,c = genetic_algorithm.activate(np.array(surgeXC),np.array(nU),4000,200,50,0.25,0.1)

# # a,b,c = genetic_algorithm.activate(np.array(surgeXC),np.array(nU),40,2,5,0.5,0.25)
...
# nU_p = predict(c,surgeXC)

# plt.plot(nU)
# plt.show()
# plt.plot(nU_p)

data1 = pd.read_csv('nU_p.csv', header = 0)
nU_p = data1.iloc[:,0][1:].tolist()

def Plot_cc(x,y):
    plt.figure(figsize=(9,6))
    plt.plot(x,'g',label = "Original")
    # plt.plot(y,'m',label="Predicted by GA")
    plt.ylabel("Δu(k+1) - Δu(k) Values")
    plt.xlabel("Surge Input Vectors")
    # plt.title("Genetic Algorithm - Prediction Comparision")
