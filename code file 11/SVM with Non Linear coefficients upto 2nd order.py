...
def Create_Swaycomponents(U,V,R,RAC,TV,L):
    ...
    """
    Sway_Components = []
    P = [list(),list(),list(),list(),list(),list(),list(),list(),list(),list()]
    for i in range(len(U)):
        c1 = V[i]*TV[i]
        c2 = R[i]*TV[i]*L
        c3 = RAC[i]*(TV[i]**2)
        c4 = (V[i]**3)/TV[i]
        c5 = (RAC[i]**3)*(TV[i]**2)
        c6 = (V[i]**2)*R[i]*L/TV[i]#1 by L
        c7 = RAC[i]*U[i]*TV[i]
        c8 = V[i]*U[i]
        c9 = R[i]*U[i]*L
        cb = TV[i]**2 #bias term
        
        temp=[c1,c2,c3,c4,c5,c6,c7,c8,c9,cb]
...
sway_hydrodynamic_derivatives = ['Y`v','Y`r',
                                  'Y`ð','Y`vvv','Y`vvr','Y`vvð',
                                  'Y`vðð','Y`ðu','Y`vu','Y`ru']

yaw_hydrodynamic_derivatives = ['N`v','N`r',
                                'N`ð','N`vvv','N`vvr','N`vvð',
                                'N`vðð','N`ðu','N`vu','N`ru']
