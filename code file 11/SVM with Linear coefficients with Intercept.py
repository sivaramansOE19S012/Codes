...
linear_svm1 = LinearSVR(C=1e08,fit_intercept = False, dual = True ,
                        ...
                        max_iter = 10000, random_state = None, verbose = 0).fit(np.array(surgeXC), np.array(nU))

coefLinear1 = linear_svm1.coef_

linear_svm1_ramp =  LinearSVR(C=1e04,fit_intercept = True, dual = True ,
                        epsilon = 1e-6, loss = 'squared_epsilon_insensitive',
...
linear_svm2 =  LinearSVR(C=1e08,fit_intercept = False, 
                        max_iter = 10000).fit(np.array(swayYC), np.array(nV))

coefLinear2 = linear_svm2.coef_

linear_svm2_ramp =  LinearSVR(C=1e04,fit_intercept = False, 
                        max_iter = 10000).fit(np.array(swayYC), np.array(nV_ramp))
...
linear_svm3 =  LinearSVR(C=1e08,fit_intercept = False, 
                        max_iter = 10000).fit(np.array(yawRC), np.array(nR))

coefLinear3 = linear_svm3.coef_

linear_svm3_ramp =  LinearSVR(C=1e04,fit_intercept = False, 
                        max_iter = 10000).fit(np.array(yawRC), np.array(nR_ramp))
...
sway_hydrodynamic_derivatives = ['Y`v','Y`r',
                                  'Y`ð','Y`ðu','Y`vu','Y`ru']

yaw_hydrodynamic_derivatives = ['N`v','N`r','N`ð','N`ðu','N`vu','N`ru']

sway_original = [-1160,-499,
                  278,556,-1160,-499]

yaw_original = [-264,-166,
                  -139,-278,-264,-166]
