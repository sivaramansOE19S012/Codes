"""
Created on Fri Nov 20 22:04:11 2020

@author: Sivaraman Sivaraj
"""
import numpy as np

a = np.load("D4_surge.npy")[:4800]
b = np.load("D4_sway.npy")[:4800]
c = np.load("D4_yaw.npy")[:4800]

xt = np.load("D4_nU.npy")[:4800]
yt = np.load("D4_nV.npy")[:4800]
zt = np.load("D4_nR.npy")[:4800]
# x = xt[12800:]
# y = yt[12800:]
# z = zt[12800:]
...
nU = EqL_next(u)[:4800]# we subtracting u(k+1) and u(k) values
nV = EqL_next(v)[:4800]
nR = EqL_next(r)[:4800]
...
linear_svm1 = LinearSVR(C= 1e04,fit_intercept = False, dual = True ,
                        ...
                        verbose = 0).fit(np.array(a), np.array(xt))
...
u_predicted = linear_svm1.predict(surgeXC_train)
...
def graph_uvr(nU_ramp_train,nV_ramp_train,nR_ramp_train,
    ...
    plt.legend(loc='best')
    plt.title("Trained with 20-20 Manuever & tested with 2.5 deg decrement Manuever")
    ...
    plt.xlabel('time in '+str(time_interval)+' seconds increment')
    plt.ylabel('yaw rate')
    # plt.savefig("2.5_deg_5_SVM_result_80_20.jpg", dpi=2400)
    plt.legend(loc='best')
...
graph_uvr(nU,nV,nR,u_predicted,v_predicted,n_predicted,0.1)
