...
a = np.load("D2_surge.npy")
b = np.load("D2_sway.npy")
c = np.load("D2_yaw.npy")

xt = np.load("D2_nU.npy")
yt = np.load("D2_nV.npy")
zt = np.load("D2_nR.npy")
x = xt[12800:]
y = yt[12800:]
z = zt[12800:]
...
data  = pd.read_csv('data 4.csv', header = 0)
"""
7*1 format
...
def graph_uvr(nU_ramp_train,nV_ramp_train,nR_ramp_train,
    ...
    plt.legend(loc='best')
    plt.title("Trained with 5 degree decrement Manuever & tested with 20_15_10_5 Manuever")
...
graph_uvr(x,y,z,u_predicted,v_predicted,n_predicted,0.1)
